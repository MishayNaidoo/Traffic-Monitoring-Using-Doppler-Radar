...
int main(void)
{
  ...
  messageQueueHandle = osMessageQueueNew (4, sizeof(uint8_t), &messageQueue_attributes);
...
void receiveTask(void *argument)
{
  ...
  for(;;)
  {
    ...
    osMessageQueueGet(messageQueueHandle, &res, NULL, osWaitForever);
    osDelay(1000);
  }
  /* USER CODE END 5 */
}
...
void transmitTask(void *argument)
{
  ...
  for(;;)
  {
    osMessageQueuePut(messageQueueHandle, &x, 0, osWaitForever);
    if(++x>9){
      x=0;
    }
    osDelay(1000);
